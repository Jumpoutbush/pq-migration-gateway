...
CREATE TABLE IF NOT EXISTS artifacts(artifact_id TEXT PRIMARY KEY,path TEXT NOT NULL,artifact_type TEXT,file_format TEXT,sha256 TEXT,size INTEGER,executable INTEGER,languages_json TEXT,dependencies_json TEXT,confidence TEXT,data_json TEXT NOT NULL,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS runtime_processes(process_id TEXT PRIMARY KEY,pid INTEGER,executable TEXT,command TEXT,mapped_libraries_json TEXT,confidence TEXT,data_json TEXT NOT NULL,updated_at TEXT NOT NULL);
...
def main()->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--db',required=True);p.add_argument('--static',action='append',default=[]);p.add_argument('--tls',default='');p.add_argument('--risk',default='');p.add_argument('--cmdb',action='append',default=[]);p.add_argument('--summary-json',default='');a=p.parse_args();db=Path(a.db);db.parent.mkdir(parents=True,exist_ok=True);now=stamp()
    with sqlite3.connect(db) as c:
        c.executescript(SCHEMA)
        for static_path in a.static:
            d=load(static_path);mark(c,'static',static_path,now)
            ...
            for x in d.get('artifacts',[]):c.execute('''INSERT INTO artifacts VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(artifact_id) DO UPDATE SET path=excluded.path,artifact_type=excluded.artifact_type,file_format=excluded.file_format,sha256=excluded.sha256,size=excluded.size,executable=excluded.executable,languages_json=excluded.languages_json,dependencies_json=excluded.dependencies_json,confidence=excluded.confidence,data_json=excluded.data_json,updated_at=excluded.updated_at''',(x['artifact_id'],x['path'],x.get('artifact_type'),x.get('file_format'),x.get('sha256'),x.get('size'),int(bool(x.get('executable'))),json.dumps(x.get('languages',[]),ensure_ascii=False),json.dumps(x.get('dependencies',[]),ensure_ascii=False),x.get('confidence'),json.dumps(x,ensure_ascii=False),now))
            for x in d.get('runtime_processes',[]):c.execute('''INSERT INTO runtime_processes VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(process_id) DO UPDATE SET pid=excluded.pid,executable=excluded.executable,command=excluded.command,mapped_libraries_json=excluded.mapped_libraries_json,confidence=excluded.confidence,data_json=excluded.data_json,updated_at=excluded.updated_at''',(x['process_id'],x.get('pid'),x.get('executable'),x.get('command'),json.dumps(x.get('mapped_crypto_libraries',[]),ensure_ascii=False),x.get('confidence'),json.dumps(x,ensure_ascii=False),now))
        if a.tls:
        ...
        c.commit();summary={'database':str(db.resolve()),'assets':c.execute('SELECT COUNT(*) FROM assets').fetchone()[0],'evidence':c.execute('SELECT COUNT(*) FROM evidence').fetchone()[0],'artifacts':c.execute('SELECT COUNT(*) FROM artifacts').fetchone()[0],'runtime_processes':c.execute('SELECT COUNT(*) FROM runtime_processes').fetchone()[0],'endpoints':c.execute('SELECT COUNT(*) FROM endpoints').fetchone()[0],'cmdb_assets':c.execute('SELECT COUNT(*) FROM cmdb_assets').fetchone()[0],'risk_findings':c.execute('SELECT COUNT(*) FROM risk_findings').fetchone()[0],'scans':c.execute('SELECT COUNT(*) FROM scans').fetchone()[0]}
