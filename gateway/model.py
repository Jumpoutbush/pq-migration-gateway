...
def normalize_config(config: dict) -> dict:
    ...
    for raw in services:
        ...
        if private_key.get("provider", "file") != "file":
            raise ConfigError("NGINX v3.7 data plane currently renders file private-key references only")
        ...
        if identity["private_key"]["provider"] != "file" and identity["certificate"]:
            raise ConfigError("NGINX v3.7 data plane currently renders file upstream identity references only")
