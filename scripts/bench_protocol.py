...
def one(a)->tuple[bool,float,str]:
    ...
    cmd=[a.openssl,'s_client','-quiet','-connect',f'{a.host}:{a.port}','-servername',a.sni,'-tls1_3','-groups',a.groups]
    if a.cafile:cmd += ['-CAfile',a.cafile,'-verify_return_error']
    if a.cert and a.key:cmd += ['-cert',a.cert,'-key',a.key]
    start=time.perf_counter();process=None;selector=selectors.DefaultSelector();output=bytearray();matched=False;error=''
    try:
        process=subprocess.Popen(cmd,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,bufsize=0)
        assert process.stdin is not None and process.stdout is not None
        selector.register(process.stdout,selectors.EVENT_READ);process.stdin.write(data);process.stdin.flush()
        expected=expect.encode();deadline=time.monotonic()+a.timeout
        while time.monotonic()<deadline:
            if expected in output:matched=True;break
            events=selector.select(timeout=min(.25,max(0.0,deadline-time.monotonic())))
            if not events:
        ...
        if not matched:error=f's_client timed out after {a.timeout} seconds' if process.poll() is None else f's_client exited with status {process.returncode}'
    ...
    if error:text=f'{text}\n{error}'.strip()
    return matched,(time.perf_counter()-start)*1000,text
